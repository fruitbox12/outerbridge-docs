...
1. ```bash
   git clone https://github.com/simplifiio/simplifi.git
   ```

2. ```bash
   cd simplifi && cd docker
   ```
...
8. You can pull from latest image by:
   ...
   ```bash
   docker pull simplifiio/simplifi
   ```
...
4. ```bash
   sudo nano /etc/nginx/conf.d/simplifi.conf
   ```
...
5. Copy paste the following and change to your domain:
   ...
   server {
       listen 80;
       listen [::]:80;
       server_name yourdomain.com; #Example: demo.simplifi.io

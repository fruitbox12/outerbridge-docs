...
## Prerequisite
- Have simplifi setup and working. See [setup guide](/get-started).
...
## Step 1
Go to `simplifi/packages/components/nodes`, create a folder `MinedTransactionWebhook`. Inside the folder create a file `MinedTransactionWebhook.ts`.
...
## Step 6
Go to `simplifi/packages/components/credentials`, create a folder `Alchemy`. Inside the folder create a file `AlchemyApi.ts`.
...
## Step 7
Back to `simplifi/packages/components/nodes/MinedTransactionWebhook/MinedTransactionWebhook.ts`, create 2 webhook methods - `createWebhook()` and `deleteWebhook()` after the constructor. 
...
## Step 11
At root path `simplifi`, run `yarn run build` to build the node into `dist` folder which will be eventually picked up by `server`. 
...
<br />
<img src="/mined-transaction-webhook.gif" alt="simplifi Mined Transaction Webhook" height="auto" width='100%'></img>
...
3. Back to simplifi, you will now see the webhook has been received and output response is available.

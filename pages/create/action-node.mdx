...
## Step 1
Go to `simplifi/packages/components/nodes`, create a folder `Etherscan`. Inside the folder create a file `Etherscan.ts`.
...
## Step 6
Go to `simplifi/packages/components/credentials`, create a folder `Etherscan`. Inside the folder create a file `EtherscanApi.ts`.
...
## Step 7
Back to `simplifi/packages/components/nodes/Etherscan/Etherscan.ts`, create `run()` function after the constructor. This function is needed for every action node.
...
<br />
<img src="/create-action-node.gif" alt="simplifi Etherscan Action Node" height="auto" width='100%'></img>

...
![Workflow1](https://raw.githubusercontent.com/fruitbox12/simplifi-docs/main/download%20(15).png)

## Workflow format

simplifi workflow uses [JSON](https://en.wikipedia.org/wiki/JSON) format. Workflow can be easily exported and loaded.
...
## Under the hood
simplifi uses [child_process](https://nodejs.org/api/child_process.html) module to spawns multiple subprocesses whenever a workflow is to be executed. 
[Node AbortController](https://www.npmjs.com/package/node-abort-controller) is used to abort subprocesses. 
Breadth First Search algorithm is used to find and execute next connected nodes. To avoid infinite loop, maximum of 3 loops is specified. This means simplifi will only executes the same loop for 3 times before terminating the execution.

[View Source Code](https://github.com/simplifiio/simplifi/blob/master/packages/server/src/ChildProcess.ts#L36)

...
<br />
<img src="/credentials.png" alt="simplifi Credentials" height="auto" width='100%'></img>
...
Encryption key will be stored under `simplifi/packages/server` as `encryption.key`. 
...
## Under the hood
simplifi uses [PBKDF2](https://en.wikipedia.org/wiki/PBKDF2) to generate an encryption key. [AES256](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) is used to encrypt/decrypt credentials.

[View Source Code](https://github.com/simplifiio/simplifi/blob/master/packages/server/src/utils/index.ts#L92)